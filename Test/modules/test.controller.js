...
const solution = () => {
      ...
      if (
        ...
        for (let i = 0; i < calculations.nod; i += 1) {
          ...
          if (
            ...
            /* If the distance of the last selected closest customer
              is greater than or equal to the current customer */
            distances[eval(calculations.lastAllocated + 1)].elements[i].distance
              .value <=
              distances[eval(calculations.lastAllocated + 1)].elements[closest]
                .distance.value &&
            /* And of the the current customer is not allocated a route */
            calculations.dosfec[i].allocated !== true
          ) {
        ...
        if (
          ...
          /* Set the total distance of the current route to be distance of this route just created  */
          calculations.prevDistance +=
            distances[eval(calculations.lastAllocated + 1)].elements[
              closest
            ].distance.value;
          /* Create a route object and remove any extra fields */
          const routeObject = {};
          ...
          routeObject.path = `${
            distances[eval(calculations.lastAllocated + 1)].elements[closest]
              .distance.text
          } from ${customers[calculations.lastAllocated]} to ${
            customers[closest]
          }.`;
          ...
          calculations.deliveryRoutes.push({
            totalLength: `${(calculations.prevDistance / 1609).toFixed(
              1
            )} miles from Store.`,
  ...
  return {
    solution: calculations.deliveryRoutes,
  };
};
